    ...
    def upsert_alert(self, alert: Dict[str, Any]) -> Dict[str, Any]:
        with self._connect() as conn:
            ...
            if existing is None:
                ...
                alert["count"] = 1
                alert["ack_ts"] = None
                self.insert_alert_event(alert_id=alert_id, action="created", actor="system", note=alert.get("message", ""))
                return alert
            ...
            self.insert_alert_event(alert_id=str(existing["id"]), action="updated", actor="system", note=alert.get("message", ""))
            return out
