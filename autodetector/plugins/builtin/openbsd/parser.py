...
def _parse_cpu_top(text: str) -> Optional[float]:
    """Parse CPU from top output (OpenBSD format)."""
    # OpenBSD top shows CPU stats differently
    m = re.search(r"CPU:\s+([\d.]+)%\s+user.*?,\s+([\d.]+)%\s+sys", text)
    if m:
    ...
    if len(vmstat_lines) >= 2:
        ...
        if len(parts) >= 4:
            us = _safe_float(parts[-4]) if parts[-4] != '-' else 0
            sy = _safe_float(parts[-3]) if parts[-3] != '-' else 0
...
def _parse_memory_vmstat(text: str) -> Optional[float]:
    ...
    # Try sysctl format
    total_match = re.search(r"hw\.physmem\s*[=:]\s*(\d+)", text)
    free_match = re.search(r"hw\.usermem\s*[=:]\s*(\d+)|vm\.stats\.vm\.v_free_count\s*[=:]\s*(\d+)", text)
    
    if total_match:
        total = _safe_float(total_match.group(1))
        if free_match:
            free_val = free_match.group(1) or free_match.group(2)
            free_mem = _safe_float(free_val)
...
def _parse_load_uptime(text: str) -> Optional[float]:
    ...
    # OpenBSD: "load average: 0.52, 0.58, 0.59"
    m = re.search(r"load average[s]?:\s*([0-9]+\.[0-9]+)", text)
    if m:
...
def _parse_interface_state(text: str) -> str:
    ...
    for iface in interfaces:
        ...
        if not lines:
            continue
        first_line = lines[0]
        # Match: "em0: flags=8843<UP,BROADCAST,RUNNING,SIMPLEX,MULTICAST> mtu 1500"
...
def parse(outputs: Dict[str, str], errors: Dict[str, str], device: Dict[str, Any]) -> Dict[str, Any]:
    """Parse OpenBSD CLI outputs into normalized metrics."""
    _ = device
