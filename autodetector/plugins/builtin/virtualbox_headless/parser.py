...
def _parse_vboxmanage_list_vms(text: str) -> int:
    """Count total VMs from VBoxManage list vms."""
    count = 0
...
def _parse_vboxmanage_running_vms(text: str) -> int:
    """Count running VMs from VBoxManage list runningvms."""
    count = 0
    for line in text.splitlines():
        if line.strip() and '"' in line:
            count += 1
...
def parse(outputs: Dict[str, str], errors: Dict[str, str], device: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # VM counts from VBoxManage
    total_vms = _parse_vboxmanage_list_vms(outputs.get("vms", ""))
    running_vms = _parse_vboxmanage_running_vms(outputs.get("running_vms", ""))
    metrics.append({"variable": "VM_COUNT", "value": float(total_vms)})
    metrics.append({"variable": "VM_RUNNING", "value": float(running_vms)})
