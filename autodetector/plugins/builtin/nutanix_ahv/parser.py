...
def _parse_acli_vm_list(text: str) -> Dict[str, int]:
    """Parse VM list from acli vm.list."""
    total_vms = 0
    running_vms = 0
    
    for line in text.splitlines():
        if line.strip() and not line.startswith("Entity"):
            parts = line.split()
            if len(parts) >= 2:
                total_vms += 1
                # Check if VM is powered on
                if "on" in line.lower() or "running" in line.lower():
                    running_vms += 1
...
def parse(outputs: Dict[str, str], errors: Dict[str, str], device: Dict[str, Any]) -> Dict[str, Any]:
    """Parse Nutanix AHV CLI outputs into normalized metrics."""
    _ = device
    ...
    # VM counts from acli
    vm_data = _parse_acli_vm_list(outputs.get("vms", ""))
